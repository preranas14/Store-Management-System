...
<div class = "Smain">
	...
	$Rusererr = $Rpasserr = $CRpasserr = $Fnameerr = $Addresserr = $Contacterr = $Categoryerr = "";
	$Idresult = 0;

if($_SERVER["REQUEST_METHOD"]=="POST")
{
	if(!empty($_POST['Ruser']) && !empty($_POST['Rpass']) && !empty($_POST['CRpass']) && !empty($_POST['Fname']) )
	{
		...
		if($password==$CPassword)
		{
			...
			$exits = mysqli_query($conn,"SELECT * FROM distributor where Email = '$username'")
            OR die("".mysqli_error($conn));
			...
			if(empty($row))
			{
				$super = mysqli_query($conn,"SELECT * FROM distributor where DID = (SELECT max(DID) from distributor) ")
                OR die("".mysqli_error($conn));
				...
				$result = mysqli_query($conn,"INSERT INTO distributor values('$Idresult','$Firstname','$username','$password','$Category','$Contact')")
									OR die("".mysqli_error($conn));
...
?>
		...
		<form class = "form1" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST">
				...
				<input class = "un" type="text" name="Fname" id="Fname" pattern = "[A-Z-a-z]*" title = "Only alphabets" placeholder = "Name" >
				...
				<select class = "un" name="Category" id="Category">
                    ...
                    <?php
                        ...
                        while($Crow = mysqli_fetch_array($category))
                        {
                            $C = $Crow['Cname'];
                            $I = $Crow['CID'];
			...
			<a href="distributorLogin.php">Already a Distributer? Login</a>
			</p>
